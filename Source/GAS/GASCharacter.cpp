...
void AGASCharacter::PossessedBy(AController* NewController)
{
	...
	if (AbilitySystemComponent)
	{
		...
		for (auto& ability : BaseAbilities)
		{
			AbilitySystemComponent->GiveAbility(FGameplayAbilitySpec(ability, 1, INDEX_NONE, this));
		}

		// fire passive effects
		for (auto& effect : PassiveGameplayEffects)
		{
			auto context = AbilitySystemComponent->MakeEffectContext();
			context.AddSourceObject(this);

			FGameplayEffectSpecHandle specHandle = AbilitySystemComponent->MakeOutgoingSpec(effect, 1, context);
